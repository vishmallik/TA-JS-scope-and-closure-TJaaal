  ...
  function test() {
    ...
    for (let i = 0; i < arrVals.length; i++) {
      ...
      for (let j = 0; j < arrCallbacks.length; j++) {
        let value = arrCallbacks[j](arrVals[i]);
        valueArray.push(value);
      }
      obj[arrVals[i]] = valueArray;
    }
    return obj;
  }
  return test();
...
function objOfMatchesWithArray(array1, array2, callback) {
  function test() {
    let obj = {};
    for (let i = 0; i < array1.length; i++) {
      let first = callback[0](array1[i]);
      for (let j = 1; j < callback.length; j++) {
        first = callback[j](first);
      }
      if (first === array2[i]) {
        obj[array1[i]] = first;
      }
  ...
  function test() {
    ...
    for (let i = 0; i < array1.length; i++) {
      ...
      for (let j = 0; j < array2.length; j++) {
        let value = array2[j](array1[i]);
        valueArray.push(value);
      }
      obj[array1[i]] = valueArray;
    }
    return obj;
  }
  return test();
...
function schedule(arr1, arr2) {
  ...
  function execute() {
    for (let i = 0; i < asrr1.length; i++) {
      setTimeout(arr1[i], arr2[i] * 1000);
    }
  }
  return check();
}
